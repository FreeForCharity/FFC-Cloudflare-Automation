...
- **Safety-table conflicts are normal, not a red flag.** Prettier reflows every row of
  `docs/workflow-safety-and-approvals.md` when a new cell widens a column, so two PRs that each
  "add one row" conflict across the whole table. Resolve by taking `main`'s table, re-inserting
  your row after its numeric neighbor, then `npx prettier --write` the file and re-run
