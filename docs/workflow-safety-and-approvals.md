...
| 502     | Google - Analytics Report (GA4 -> JSON)            | Reads                     | google-prod-read / ✅ github-prod                            | delivers JSON to ffcadmin via PR (CBM_TOKEN, github-prod approval); PII-safe aggregates                                                                                                  |
...
| 736     | Repo - Archive / Application Denied (Admin)        | Writes (dry-run default)  | ✅ github-prod                                               | archive-only (reversible, never deletes); `dry_run` default true; typed `confirm_repo`; org-locked + denylist                                                                            |
