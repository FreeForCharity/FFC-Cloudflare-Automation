...
7. **Editing an already-tested step? Update its fixture in the same PR.** The workflow-logic
   harness extracts the *live* script from the YAML, so changing a step's bash (new file copied,
   new env var, new `gh` subcommand) breaks that module's fixtures — and it surfaces only in the
   merge group, after review. Before editing a workflow, grep `tests/workflow-logic/` for its file
   name; if a module extracts the step you're touching, extend its fixture seeding/shim in the same
   PR (e.g. #732 added a `cp ../agentic-os-status.json …` to the 502 deliver step that
