...
function Format-WhmcsDateParam {
...
try {
    $api = Get-WhmcsApiUrl -ApiUrlParam $ApiUrl
    $creds = Get-WhmcsCredentials -IdentifierParam $Identifier -SecretParam $Secret
    $accessKey = Get-WhmcsAccessKey -AccessKeyParam $AccessKey
    ...
    while ($true) {
        ...
        $sd = Format-WhmcsDateParam -Date $StartDate
        if ($sd) { $body.date = $sd }

        $ed = Format-WhmcsDateParam -Date $EndDate
