...
function Normalize-DomainName {
    ...
    if ($d.StartsWith('http://') -or $d.StartsWith('https://')) {
        try {
            ...
            # If it's not a parseable URL, leave it.
        }
    }

    # Remove any leading wildcard/subdomain formatting the user might pass.
    $d = $d.Trim('.')
    return $d
}
...
function Find-WhmcsDomain {
    ...
    while ($true) {
        ...
        if ($r.domains) {
            ...
            # Fall back to the flat format used by some WHMCS installs.
            $rx = '^domains\[domain\]\[(\d+)\]\[([^\]]+)\]$'
            $byIndex = @{}
...
try {
    ...
    if (-not $existing) {
        throw "Domain '$domainName' not found in WHMCS (GetClientsDomains)."
    }
    ...
    if ([string]::IsNullOrWhiteSpace($domainId)) {
        $diag = $null
        try { $diag = ($existing | ConvertTo-Json -Depth 6 -Compress) } catch {}
        throw "WHMCS domain record did not include an id/domainid field. Record: $diag"
    }

    $ns1 = if ($NameServer1) { $NameServer1.Trim().ToLowerInvariant() } else { $env:WHMCS_NAMESERVER_1 }
    $ns2 = if ($NameServer2) { $NameServer2.Trim().ToLowerInvariant() } else { $env:WHMCS_NAMESERVER_2 }
    $ns3 = if ($NameServer3) { $NameServer3.Trim().ToLowerInvariant() } else { $env:WHMCS_NAMESERVER_3 }
    $ns4 = if ($NameServer4) { $NameServer4.Trim().ToLowerInvariant() } else { $env:WHMCS_NAMESERVER_4 }
    $ns5 = if ($NameServer5) { $NameServer5.Trim().ToLowerInvariant() } else { $env:WHMCS_NAMESERVER_5 }

    if ([string]::IsNullOrWhiteSpace($ns1) -or [string]::IsNullOrWhiteSpace($ns2)) {
    ...
    $bodyUpdate = @{}
    foreach ($k in $auth.Keys) { $bodyUpdate[$k] = $auth[$k] }
    $bodyUpdate.action = 'UpdateClientDomain'
    $bodyUpdate.responsetype = 'json'
    $bodyUpdate.domainid = $domainId
    $bodyUpdate.ns1 = $ns1
    $bodyUpdate.ns2 = $ns2
    if ($ns3) { $bodyUpdate.ns3 = $ns3 }
    if ($ns4) { $bodyUpdate.ns4 = $ns4 }
    if ($ns5) { $bodyUpdate.ns5 = $ns5 }

    [void](Invoke-WhmcsApi -ApiUrl $api -Body $bodyUpdate)
    Write-Host "Updated WHMCS nameservers for '$domainName' (domainId=$domainId)." -ForegroundColor Green
    Write-Host "- ns1: $ns1"
    Write-Host "- ns2: $ns2"
    exit 0
}
