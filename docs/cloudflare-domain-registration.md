...
| `.github/workflows/113-cloudflare-domain-register.yml`        | Workflow 12: search / check / register, plus a label-triggered check (safety-gated). |
| `scripts/cloudflare-registrar-access-check.ps1`               | Read-only probe: does the token have Registrar read/write?                           |
| `.github/workflows/114-cloudflare-registrar-access-check.yml` | Manual workflow to validate Registrar API rights.                                    |
...
To confirm whether the current token already has these rights, run **"114. Domain - Validate
...
Run **"113. Domain - Registrar Search / Check / Register (Admin, DRAFT) [CF]"** from the Actions
tab:
