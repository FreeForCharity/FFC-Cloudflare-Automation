...
## Objective
Refactor the existing Python-based DNS automation (`update_dns.py`) into a unified, robust PowerShell module/script (`Update-CloudflareDns.ps1`). This change aims to simplify GitHub Actions workflows (removing Python dependency management) and align tooling with the primary development environment.
...
- **Feature Gaps**: The current Python script lacks support for MX and TXT records, which are required for "New Domain" workflows.
- **Consolidation**: Merge the logic of `update_dns.py` and `Update-StagingDns.ps1` into a single, capable tool.

## Scope & Requirements
The new PowerShell script must replicate all `update_dns.py` functionality and expand upon it to support all undefined use cases in the Issue Templates.
...
- [ ] **Authentication**: Support token via parameter and environment variable (`CLOUDFLARE_API_TOKEN`).
...
- [ ] **Zone Lists**: Ability to list zones/records without finding a specific ID first (search improvement).
...
- [ ] (Optional) Script `Export-CloudflareZone` to CSV (porting `export_zone_dns_summary.py` logic) *before* removal.
