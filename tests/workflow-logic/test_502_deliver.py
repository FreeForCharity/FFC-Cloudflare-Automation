...
def _seed_target_files(
    root: pathlib.Path, ga_body: str, catalog_body: str, status_body: str
) -> None:
    """Lay out the six synced paths (2 GA reports + 2 workflow-catalog copies +
    2 agentic-os-status copies) the deliver step writes, under a repo working
    tree at `root`."""
    ...
    _write(root / "src" / "data" / "agentic-os-status.json", status_body)
    _write(root / "public" / "data" / "agentic-os-status.json", status_body)
...
def _build_origin(td: pathlib.Path, *, sync_branch: bool, sync_matches_source: bool) -> pathlib.Path:
    ...
    if sync_branch:
        ...
        if sync_matches_source:
            _seed_target_files(seed, SOURCE_GA, SOURCE_CATALOG, SOURCE_STATUS)
        else:
            _seed_target_files(
                seed, '{"stale":"old"}\n', '{"catalog":"old"}\n', '{"status":"old"}\n'
            )
...
def run_deliver(*, sync_branch: bool, sync_matches_source: bool, gh_env: dict | None = None):
    ...
    # The status feed is generated by the preceding workflow step into the
    # checkout root (../agentic-os-status.json relative to ./target).
    _write(work / "agentic-os-status.json", SOURCE_STATUS)
...
def test_source_fixtures_are_self_consistent():
    ...
    assert json.loads(SOURCE_STATUS)
