...
| 215     | WHMCS - Nonprofit Clients Metrics (svc)   | Reads                     | ✅ whmcs-prod                                              | aggregate counts + product names only (no PII)                                      |
| 216     | WHMCS - Activity Metrics (full history)   | Reads                     | ✅ whmcs-prod                                              | aggregate counts only (no PII)                                                      |
| 217     | WHMCS - Client Fields Survey              | Reads                     | ✅ whmcs-prod                                              | throttled per-client loop; vocabulary-matched values only (no PII)                  |
| 218     | WHMCS - Sites-List Reconcile + Products   | Reads                     | ✅ whmcs-prod                                              | aggregate summary; artifact carries org-domain gap lists only                       |
| 219     | WHMCS - Application Detail (one client)   | Reads                     | ✅ whmcs-prod                                              | single-client projection; personal contact fields always masked                     |
