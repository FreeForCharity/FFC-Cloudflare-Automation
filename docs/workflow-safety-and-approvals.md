...
| 04    | Domain - Export Inventory                 | Reads                     | ✅ whmcs-prod / ✅ m365-prod / ✅ wpmudev-prod (+ cf-read) | waits on whmcs-prod, m365-prod, and wpmudev-prod approvals                          |
...
| 47    | Discover - Uncaptured Comms (M365)        | Reads                     | ✅ m365-prod                                               | PII masked; dispatch-only; org mailboxes only; waits on m365-prod approval          |
| 48    | Google - API Smoke (GA4 connectivity)     | Reads                     | google-prod-read                                           | read-only; fails closed; reusable via `workflow_call`                               |
| 49    | Google - Analytics Report (GA4 -> JSON)   | Reads                     | google-prod-read                                           | delivers JSON to ffcadmin via PR (CBM_TOKEN); PII-safe aggregates                   |
| 51    | Google - GTM Provision (per-charity)      | Writes (dry-run default)  | ✅ google-prod-write                                       | dry_run default true; seeds GA4/Clarity/Meta; delegates POC access                  |
