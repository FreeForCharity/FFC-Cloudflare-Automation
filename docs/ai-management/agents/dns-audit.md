...
Audit Cloudflare DNS records for FFC charity domains. Verify that all required records are present
and correctly configured for GitHub Pages hosting, Microsoft 365 email, and security best practices.

## Context

Free For Charity manages ~30 domains across two Cloudflare accounts (FFC and CM). Each charity
domain follows a standard DNS configuration pattern for GitHub Pages hosting and Microsoft 365
email. This agent checks whether a domain's DNS records match that standard.

## Instructions

When invoked, ask for the domain name to audit (or accept it as an argument). Then perform the
following checks:
