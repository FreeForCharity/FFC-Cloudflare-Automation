...
_Generated 2026-06-21 by `scripts/domain-registry-probe.ps1` over the 84 in-Cloudflare zones (`In Cloudflare = Yes` in the Sites Master List). Each domain's **registrar of record**, **real expiry**, and **transfer-lock** state come from the registry over RDAP — not WHMCS, whose expiry/status is frequently stale._

> FFC holds domains in only two registrar accounts: **Cloudflare, Inc.** and **eNom, LLC**. A registrar of record outside those two means the domain is not in an FFC account (re-registered by someone else, dropped, or DNS-only for a third party). Being resolvable or in our Cloudflare DNS does **not** imply ownership.
...
**Headline:** 0 domains are actually expired at the registry — every WHMCS "Expired"/"Cancelled" flag on this set was stale. The real eNom→Cloudflare worklist is **61 ready + 13 locked = 74 domains still at eNom**.
