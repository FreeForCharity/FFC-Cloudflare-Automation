...
| `whmcs_application_detail` | `GetClientsDetails` + `GetOrders` + `GetClientsProducts` (one id) | `scripts/whmcs-application-detail.ps1` (wf 219) | Read one application's readable fields (mission, desired domain, legal status).                                                                     |
| `whmcs_client_products`    | `GetClientsProducts`                                              | `scripts/whmcs-application-detail.ps1` (subset) | List a client's services with product custom fields (names + masked values) — just the `GetClientsProducts` slice of the application-detail script. |
